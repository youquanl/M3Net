...
def handle_process(file_path, output_root):
    ...
    for data in data_group:
        ...
        if frame.lasers[0].ri_return1.segmentation_label_compressed:
            ...
            point_cloud.astype(np.float32).tofile(os.path.join(save_path, "velodyne", f"{file_idx}.bin"))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--splits', required=True, nargs="+", choices=['training', 'validation', 'testing'],
    ...
    # load file list
    file_list = glob.glob(os.path.join(os.path.abspath(config.dataset_root), "*", "*.tfrecord"))
