...
class DefaultDataset(Dataset):
    def __init__(self,
        ...
        if test_mode:
            ...
            if self.test_cfg.panoptic_mode:
                self.pano_transform = Compose(self.test_cfg.pano_transform)
    ...
    def prepare_test_data(self, idx):
        ...
        data_dict = self.get_data(idx)
        if self.test_cfg.panoptic_mode:
            pano_dict = deepcopy(data_dict)
            pano_dict = self.pano_transform(pano_dict)
        data_dict = self.transform(data_dict)
        ...
        if self.test_cfg.panoptic_mode:
           result_dict["pano_dict"] = pano_dict
        return result_dict
