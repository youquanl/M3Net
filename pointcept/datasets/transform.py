...
class Collect(object):
    ...
    def __call__(self, data_dict):
        ...
        for name, keys in self.kwargs.items():
            ...
            data[name] = torch.cat([data_dict[key].float() for key in keys], dim=1) #拼接xyz & i
        return data
...
@TRANSFORMS.register_module()
class Offset_nusc(object):
    def __init__(self, condition):
        self.cond = condition

    def __call__(self, data_dict):
        offsets = np.zeros([data_dict['org_coord'].shape[0], 3], dtype=np.float32)
        xyz = data_dict['org_coord'][:, :3]
        ins_labels = data_dict['inst_data']
        things_ids = set([9, 14, 15, 16, 17, 18, 21, 2, 3, 4, 6, 12, 22, 23])

        for i in np.unique(ins_labels):
            if (i // 1000) not in things_ids:
                continue
        ...
        data_dict['gt_off'] = offsets

        return data_dict
    
@TRANSFORMS.register_module()
class Offset_kitti(object):
    def __init__(self, condition):
        self.cond = condition
...
class GridSample(object):
    ...
    def __call__(self, data_dict):
        ...
        data_dict["org_coord"] = data_dict["coord"]
        if self.mode == 'train':  # train mode
            ...
            if self.return_discrete_coord or self.return_grid_coord:  # True
                data_dict["discrete_coord"] = discrete_coord[idx_unique]
                data_dict["grid_coord"] = discrete_coord[idx_unique]
            ...
            for key in self.keys:
                data_dict[key] = data_dict[key][idx_unique] # TODO
            data_dict["inverse_indexes"] = np.zeros_like(inverse)
            data_dict["inverse_indexes"][idx_sort] = inverse
            if "pairing_points" in data_dict.keys():
                data_dict["pairing_points"] = data_dict["inverse_indexes"][data_dict["pairing_points"]]
                # data_dict["inverse_indexes"] = inverse
            ...
            for i in range(count.max()):
                ...
                if self.return_inverse:
                    data_dict["inverse"] = np.zeros_like(inverse)
                    data_dict["inverse"][idx_sort] = inverse
                if self.return_discrete_coord or self.return_grid_coord:
                    data_part["discrete_coord"] = discrete_coord[idx_part]
                    data_part["grid_coord"] = discrete_coord[idx_part]
                if self.return_min_coord:
