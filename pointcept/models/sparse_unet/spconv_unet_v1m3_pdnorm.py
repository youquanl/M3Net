    ...
    def __init__(self,
        ...
        if self.decouple:
            self.bns = nn.ModuleList([
                nn.BatchNorm1d(num_features=num_features, eps=eps, momentum=momentum, affine=affine)
                for _ in conditions
            ])
        else:
...
class BasicBlock(spconv.SparseModule):
    ...
    def __init__(self,
        ...
        if in_channels == embed_channels:
            ...
            self.proj_conv = spconv.SubMConv3d(in_channels, embed_channels, kernel_size=1, bias=False)
    ...
    def forward(self, x):
        ...
        if self.in_channels == self.embed_channels:
            ...
            residual = residual.replace_feature(self.proj_norm(
...
class SPConvDown(nn.Module):
    def __init__(self,
        ...
        self.conv = spconv.SparseConv3d(
            in_channels, out_channels, kernel_size=kernel_size, stride=kernel_size, bias=bias, indice_key=indice_key
        )
...
class SPConvPatchEmbedding(nn.Module):
    def __init__(self,
        ...
        self.conv = spconv.SubMConv3d(
            in_channels, out_channels, kernel_size=kernel_size, padding=1, bias=False, indice_key='stem'
        )
        self.bn = norm_fn(out_channels)
        self.relu = nn.ReLU()
...
class SpUNetBase(nn.Module):
    def __init__(self,
        ...
        norm_fn = partial(PDBatchNorm,
                          eps=1e-3, momentum=0.01, conditions=conditions, context_channels=context_channels,
                          decouple=norm_decouple, adaptive=norm_adaptive, affine=norm_affine)
        block = BasicBlock

        self.conv_input = SPConvPatchEmbedding(in_channels, base_channels, kernel_size=5, norm_fn=norm_fn)
        ...
        for s in range(self.num_stages):
            # encode num_stages
            self.down.append(SPConvDown(
                enc_channels, channels[s], kernel_size=2, bias=False, indice_key=f"spconv{s + 1}", norm_fn=norm_fn
            ))
            ...
            if not self.cls_mode:
                # decode num_stages
                self.up.append(SPConvUp(
                    channels[len(channels) - s - 2], dec_channels, kernel_size=2, bias=False,
                    indice_key=f"spconv{s + 1}", norm_fn=norm_fn
                ))
        ...
        self.feat_ = spconv.SubMConv3d(final_in_channels, 256, kernel_size=1, padding=1, bias=True)
