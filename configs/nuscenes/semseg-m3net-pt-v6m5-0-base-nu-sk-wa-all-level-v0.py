...
data = dict(
    ...
    val=dict(
        ...
        transform=[
            ...
            dict(
                type="Collect",
                keys=("coord", "grid_coord", "segment", "condition","inverse_indexes", "gt_off", "inst_data","valid", "org_coord"),
                feat_keys=("coord", "strength"),
            ),
        ],
        test_mode=False,
    ...
    test=dict(
        ...
        test_cfg=dict(
            ...
            post_transform=[
                ...
                dict(
                    type="Collect",
                    keys=("coord", "grid_coord", "index", "condition","inverse_indexes", "inst_data","valid", "org_coord"),
                    feat_keys=("coord", "strength"),
                ),
            ],
            pano_transform=[
                dict(type="GridSample", grid_size=0.1, hash_type="fnv", mode="train",
                 keys=("coord", "strength", "segment"),return_grid_coord=True),
                dict(type="Add", keys_dict={"condition": "nuScenes", "panoptic_eval":True}),
                dict(type="Offset_nusc", condition="nuScenes"),
                dict(type="ToTensor"),
                dict(type="Collect",
                     keys=("coord", "grid_coord", "discrete_coord",  "condition","inverse_indexes", "org_coord","gt_off", "inst_data","valid"),
                     feat_keys=("coord", "strength"))
            ],
            aug_transform=[
